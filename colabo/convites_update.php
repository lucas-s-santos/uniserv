...
<section class="collab-calls">
    <div class="collab-calls__header">
        <div class="collab-calls__info">
            <div class="section-title">📞 Chamados pendentes</div>
            <p class="section-subtitle">Responda rapido para manter sua reputacao alta</p>
        </div>
        <form action="#" method="POST" class="collab-city-form">
            <?php 
                ...
                if ($resultado5['cidade'] == " " || $resultado5['cidade'] == "") {
                    echo "<div class='notice notice--warn' style='margin-top: 8px;'>Você precisa digitar sua cidade para receber chamados.</div>";
                }
            ?>
        </form>
    </div>
    ...
    <?php
        function show_call_notice($message) {
            ...
            echo "<div class='collab-empty'>
                <div class='collab-empty__icon'>📭</div>
                <div class='collab-empty__text'>{$safe}</div>
                <p class='collab-empty__hint'>Novos chamados aparecerao aqui automaticamente</p>
            </div>";
        }

        $status_pendente = SERVICO_STATUS_PENDENTE;
        $verifique = "SELECT A.endereco 'endereco', B.nome_func 'funcao', A.id_servico 'id_servico',
            A.valor_atual 'valor', A.data_2 'data_2', C.nome 'nome', C.foto 'foto_cliente', C.telefone 'telefone'
            ...
            ORDER BY A.data_2 DESC, A.id_servico DESC";
        $jogue_no_banco = mysqli_query($conn, $verifique);
        $total_chamados = mysqli_num_rows($jogue_no_banco);
        $tem_chamado = false;
        
        if ($total_chamados > 0) {
            echo "<div class='collab-calls__alert'>
                    <div class='alert-card alert-card--info'>
                        <div class='alert-card__icon'>🔔</div>
                        <div class='alert-card__content'>
                            <div class='alert-card__title'>Voce tem {$total_chamados} chamado(s) aguardando resposta</div>
                            <div class='alert-card__text'>Aceite rapidamente para garantir o trabalho e melhorar sua avaliacao</div>
                        </div>
                    </div>
                </div>";
        }
        
        echo "<div class='collab-calls__grid'>";
        while ($linha75 = mysqli_fetch_array($jogue_no_banco)) {
            $tem_chamado = true;
            $data_formatada = $linha75['data_2'] ? date('d/m/Y \à\s H:i', strtotime($linha75['data_2'])) : 'Agora';
            ...
            $telefone = !empty($linha75['telefone']) ? htmlspecialchars($linha75['telefone'], ENT_QUOTES, 'UTF-8') : 'Nao informado';
            
            echo "<div class='call-card call-card--highlight'>
                <div class='call-card__status-bar'></div>
                <div class='call-card__header'>
                    <div class='call-card__client'>
                        <div class='call-card__avatar-wrapper'>
                            <img class='call-card__avatar' src='{$foto_cliente_safe}' alt='Cliente'>
                            <span class='call-card__badge'>Novo</span>
                        </div>
                        <div class='call-card__client-info'>
                            <div class='call-card__client-name'>{$nome}</div>
                            <div class='call-card__service-type'>🛠️ {$funcao}</div>
                        </div>
                    </div>
                </div>
                
                <div class='call-card__details'>
                    <div class='call-card__detail-item'>
                        <span class='call-card__icon'>📍</span>
                        <div>
                            <div class='call-card__label'>Endereco</div>
                            <div class='call-card__value'>{$endereco}</div>
                        </div>
                    </div>
                    <div class='call-card__detail-item'>
                        <span class='call-card__icon'>💰</span>
                        <div>
                            <div class='call-card__label'>Valor por hora</div>
                            <div class='call-card__value call-card__value--highlight'>R$ {$valor}</div>
                        </div>
                    </div>
                    <div class='call-card__detail-item'>
                        <span class='call-card__icon'>📅</span>
                        <div>
                            <div class='call-card__label'>Solicitado em</div>
                            <div class='call-card__value'>{$data_formatada}</div>
                        </div>
                    </div>
                    <div class='call-card__detail-item'>
                        <span class='call-card__icon'>📱</span>
                        <div>
                            <div class='call-card__label'>Telefone</div>
                            <div class='call-card__value'>{$telefone}</div>
                        </div>
                    </div>
                </div>
                ...
                <form action='#' method='POST' class='call-card__actions'>
                    ...
                    <button type='submit' name='escolha' value='sim' class='btn btn-success btn-block'>
                        ✓ Aceitar chamado
                    </button>
                    <button type='submit' name='escolha' value='nao' class='btn btn-ghost btn-block'>
                        ✗ Recusar
                    </button>
                </form>
            </div>";
